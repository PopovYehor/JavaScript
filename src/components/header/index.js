import "./style.scss"
import { onHandleRoute } from "../../helper/route";
const header = () =>{
    const head = `
    <header id="head">
                    <div class="first-level">
                        
                        <div class="title">
                            <div class="title-company">
                            <svg width="30" height="30" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
                                <rect width="30" height="30" fill="url(#pattern0)"/>
                                <defs>
                                <pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
                                <use xlink:href="#image0_68_417" transform="translate(-0.233333 -0.218333) scale(0.0216667)"/>
                                </pattern>
                                <image id="image0_68_417" width="300" height="94" xlink:href="data:image/png;base64,"/>
                                </defs>
                                </svg>
                                <a href = "/"class="name-company-title">ОфисDирект</a>
                            </div>
                                <h2 class="name-company-description">КРЕСЛА SAMURAI</h2>
                        </div>

                        <div class="search">
                            <input class="search-input" type="text" placeholder="&#xf002;  Название модели или артикул" style="font-family:Arial, FontAwesome">
                        </div>

                        <div class="phone">
                            <span class="phone-number"> &#xf095; +380 99-047-58-69</span>
                        </div>

                        <div class="call">
                            <a href="#" class="call-text">Заказать звонок</a>
                        </div>

                    </div>

                    <div class="second-level">

                        <ul class="nav-list">
                            <a href="/catalog" class="catalog nav-list-items">&#xf03a; Каталог</a>
                            <a href="/about-product" class="about-product nav-list-items">О продукте</a>
                            <a href="/reviews" class="reviews nav-list-items">Отзывы</a>
                            <a href="/delivery" class="delivery nav-list-items">Доставка и оплата</a>
                            <a href="/contact" class="contact nav-list-items">Контакты</a>
                        </ul>

                        <div class="loc-basket">
                            <div class="location">
                                <span class="location-icon">&#xf1d8;</span>
                                <select name="local" id="locations">
                                    <option value="Dnipro" selected>Днепр</option>
                                </select>
                            </div>

                            <div class="basket">
                                <a href="/basket" class="basket-item">&#xf07a; Моя корзина <span class="basket-count">+1</span></a>
                            </div>
                        </div>

                    </div>
            </header>
    `
    const root = document.getElementById('root')
    root.innerHTML = head

    const catalog = document.querySelector('.catalog')
    catalog.addEventListener('click', onHandleRoute)

    const aboutProduct = document.querySelector('.about-product')
    aboutProduct.addEventListener('click', onHandleRoute)

    const reviews = document.querySelector('.reviews')
    reviews.addEventListener('click', onHandleRoute)

    const delivery = document.querySelector('.delivery')
    delivery.addEventListener('click', onHandleRoute)

    const basket = document.querySelector('.basket-item')
    basket.addEventListener('click', onHandleRoute)

    const title = document.querySelector('.name-company-title')
    title.addEventListener('click', onHandleRoute)
}

export {header}